...
(LINE_ID, VNO_CODE, ID_DIVISOR, CODIGO, CODIGO_SITIO, CODIGO_DIV)
    ...
    WHERE B.LINE_ID IS NOT NULL 
		AND B.LINE_ID <> '' 
		--AND ID_ESTADO_PROYECTO IN (4,6) -- ESTADO DE PROYECTO 4 : DATOS PERMANENTES / 6 : EXISTENTES
...
BEGIN  
	INSERT INTO @tTrace
	(Trace, DIVISOR_N1, ID_DIVISOR, CODIGO_SITIO, LastCableID, LastNodeID,SENAL,CUENTA)
	SELECT Trace, DIVISOR_N1, @ID_DIVISOR, @CODIGO_SITIO, LastCableID, LastNodeID,SENAL,CUENTA
		FROM
		(
			SELECT STRING_AGG(CONCAT(Puerto2.ID_EQUIPO,'|',Puerto2.ID_TIPO_EQUIPO,'|',fi.ID_TRAMO_CABLE,'|',SENAL,'|',fi.CUENTA), '&&') WITHIN GROUP (GRAPH PATH) AS Trace,
				...
				LAST_VALUE(fi.SENAL) WITHIN GROUP (GRAPH PATH) AS SENAL,
...
SELECT p.NOMBRE_COMPLETO as CodigoPuertoODF, C.CODIGO as CODIGO_CABLE_ODF, 
		b.LINE_ID, b.VNO_CODE, b.CODIGO, b.CODIGO_DIV, b.ID_DIVISOR, b.CODIGO_SITIO,C.FIBRAS,Q.SENAL,Q.CUENTA
FROM @tTrace AS Q inner join @tBorne b
	on q.ID_DIVISOR = b.ID_DIVISOR
	and q.CODIGO_SITIO = b.CODIGO_SITIO
	and q.ID_DIVISOR = 213972
...
SELECT DISTINCT F.SENAL+'_'+CONVERT(VARCHAR,F.CUENTA) FROM RED.FIBRA F
WHERE F.CUENTA IS NOT NULL
AND F.SENAL LIKE '%ZA%'
AND F.SENAL = 'ZAG306D03'
